...
int state = 0;  //	0 representa o menu, 1 representa jogo, 2 representa derrota
                //no jogo e qualquer outro representa encerramento do jogo
...
GLfloat view_w = WINDOW_WIDTH / 2,
        view_h =
            WINDOW_HEIGHT /
            2;  //  Os valores representam a metade do tamanho atual da janela
...
pair<GLfloat, GLfloat> p0(
    -view_h * 0.1,
    -view_h * 0.1);  //	Ponto inferior esquerdo retangulo do menu
pair<GLfloat, GLfloat> p1(
    view_h * 0.3, view_h * 0.03);  //	Ponto superior direito retangulo do menu
//	Map usado para determinar a opcao selecionada que ira configurar o jogo
unordered_map<string, bool> option({{"NORMAL1", 1},
                                    {"RAPIDO", 0},
                                    {"TURBO", 0},
                                    {"20x10", 1},
                                    {"30x15", 0},
...
unordered_map<string, pair<int, bool>> selecN({{"INICIAR", make_pair(0, 1)},
                                               {"NORMAL1", make_pair(1, 0)},
                                               {"RAPIDO", make_pair(2, 0)},
                                               {"TURBO", make_pair(3, 0)},
                                               {"20x10", make_pair(4, 0)},
                                               {"30x15", make_pair(5, 0)},
                                               {"50x25", make_pair(6, 0)},
                                               {"Cores1", make_pair(7, 0)},
                                               {"Cores2", make_pair(8, 0)},
                                               {"Cores3", make_pair(9, 0)},
                                               {"NORMAL2", make_pair(10, 0)},
                                               {"BEBADO", make_pair(11, 0)},
                                               {"SAIR", make_pair(12, 0)}});
//  Map usado para mapear o numero do botao ao nome
unordered_map<int, string> selecI({{0, "INICIAR"},
                                   {1, "NORMAL1"},
                                   {2, "RAPIDO"},
                                   {3, "TURBO"},
                                   {4, "20x10"},
...
//  Vetor com o nome dos botoes, NORMAL1 e NORMAL2 sao excecoes pois causam
//  conflito ao usar um map
const vector<string> optNames{
    "INICIAR", "NORMAL1", "RAPIDO", "TURBO",   "20x10",  "30x15", "50x25",
    "Cores1",  "Cores2",  "Cores3", "NORMAL2", "BEBADO", "SAIR"};
...
string cor =
    (option["Cores1"]) ? "Cores1" : (option["Cores2"]) ? "Cores2" : "Cores3";

// -------- Variaveis do jogo --------

GLfloat spin = 0.0;  //	Define a rotacao do tabuleiro durante o modo bebado
pair<int, int> tam;  //	Define o tamanho do tabuleiro
int vel, altura, alturaOld, pontos;    //	Define a velocidade, altura, altura
                                       //anterior e pontos respectivamente
Tetris jogo(0), jogoComPecaCaindo(0);  //	Define dois objetos do Tetris
const int possiveisRotacoes[] = {0, 90, 180,
                                 270};  //	Define as rotacoes disponiveis
int larguraJogo, alturaMaximaJogo, alturaPecaAtual, posicaoPecaAtual,
    rotacaoPecaAtual;           //	Auxiliares
