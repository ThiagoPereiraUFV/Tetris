...
string mousePointer(const GLint x, const GLint y) {
	for(auto key : optNames) {
		if(key == "TURBO" || key == "50x25" || key == "Cores3" || key == "BEBADO") {
			if(x >= p0.first+boxPos[key].first && x <= p1.first+boxPos[key].first && y >= p0.second+boxPos[key].second && y <= p1.second+boxPos[key].second)
				return key;
		}
		else	if(x >= p0.first+boxPos[key].first && x <= p1.first+boxPos[key].first && y >= p0.second+boxPos[key].second && y <= p1.second+boxPos[key].second)
					return key;
	}
	return "";
}
...
void drawText(const GLint x, const GLint y, const GLfloat sx, const GLfloat sy, const string text) {
	...
	if(out == "NORMAL1" || out == "NORMAL2")
		out = "NORMAL";
	glColor3f(colors[cor]["Text"][0], colors[cor]["Text"][1], colors[cor]["Text"][2]);
	glPushMatrix();
		glTranslatef(x, y, 0);
		glScalef(sx, sy, 1.0);
...
void drawButton(const pair<GLfloat, GLfloat> &pos, const string text) {
	...
	glPushMatrix();
		...
		glColor3f(colors[cor]["Box"][0], colors[cor]["Box"][1], colors[cor]["Box"][2]);
		glPushMatrix();
			glTranslatef(pos.first, pos.second, 0);
			glRecti(p0.first, p0.second, p1.first, p1.second);
		glPopMatrix();
		if(option[text]) {
			glPushMatrix();
				glColor3f(colors[cor]["Background"][0], colors[cor]["Background"][1], colors[cor]["Background"][2]);
				glTranslatef(pos.first, pos.second, 0);
				glRecti(p0.first, p0.second, p1.first, p1.second);
			glPopMatrix();
		}
